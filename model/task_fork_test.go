...
func TestForkTask_UnmarshalJSON(t *testing.T) {
	jsonData := `{
		"fork": {
			"branches": [
	...
	var forkTask ForkTask
	err := json.Unmarshal([]byte(jsonData), &forkTask)
	assert.NoError(t, err)
	assert.Equal(t, true, forkTask.Fork.Compete)

	task1 := forkTask.Fork.Branches.Key("task1").AsCallHTTPTask()
	...
	task2 := forkTask.Fork.Branches.Key("task2").AsCallOpenAPITask()
...
func TestForkTask_MarshalJSON(t *testing.T) {
	forkTask := ForkTask{
		TaskBase: TaskBase{},
	...
	data, err := json.Marshal(forkTask)
	assert.NoError(t, err)
	assert.JSONEq(t, `{
		"fork": {
			"branches": [
...
func TestForkTask_Validation(t *testing.T) {
	forkTask := ForkTask{
		TaskBase: TaskBase{},
		Fork: ForkTaskConfiguration{
			Branches: &TaskList{
